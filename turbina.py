...
try:
    while True:
        ...
        TUR1.motorAux = True
        TUR1.update()
        print(f"VEL: {TUR1.RPM:.2f}, VALV: {TUR1.Valvula:.2f}, Estado {estado}")
        time.sleep(0.5)
        if TUR1.RPM > 478.0 and TUR1.RPM < 600.0:
            estado += 1
        if estado == 2:
            TUR1.Q1 = True
            TUR1.Q2 = True
            ...
            TUR1.modoControl = True
            TUR1.PID(input=TUR1.RPM, Man_Auto=False, SetpointAuto=4600.0)
            TUR1.update()    
            print(f"VEL: {TUR1.RPM:.2f}, VALV: {TUR1.Valvula:.2f}, Estado {estado}")
            time.sleep(0.5)
